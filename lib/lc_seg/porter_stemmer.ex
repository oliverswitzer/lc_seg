defmodule LcSeg.PorterStemmer do
  @moduledoc """
  Taken from https://github.com/frpaulas/porterstemmer
  """
  ...
  def meq1(),
    do: ~r/^(#{consonant_sequence})?#{vowel_sequence}#{consonant_sequence}(#{vowel_sequence})?$/
  ...
  def suffix_1_regex(w) do
    Regex.run(
      ~r/(\w+)(ational|tional|enci|anci|izer|bli|alli|entli|eli|ousli|ization|ation|ator|alism|iveness|fulness|ousness|aliti|iviti|biliti|logi)$/r,
      w
    )
  end
  ...
  def suffix_2_regexp(w) do
    Regex.run(
      ~r/(\w+)(al|ance|ence|er|ic|able|ible|ant|ement|ment|ent|ou|ism|ate|iti|ous|ive|ize)$/r,
      w
    )
  end
